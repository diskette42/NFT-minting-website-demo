  ...
  const searchImageByAsset =(name)=>{
    for (let i=0;i<datas.length;i++){
      if(datas[i].name == name ){
        setAssetChoice({...assetChoice,asset:datas[i].asset,name:datas[i].name})
        ...
        <div className="flex justify-center flex-row w-full">
            ...
            <div className="flex flex-col justify-start items-start w-4/6">
              <div className='flex flex-wrap'>
                {datas.map((data,i)=>(
                  <div onClick={()=>setAsset(data.name)} key={i} className={`border-2 px-10 mr-5 my-2 cursor-pointer ${asset==data.name ?"bg-rose-300 border-rose-300" : "border-rose-300"}`}>
                    {data.name}
                  </div>
                ))}
              </div>
              ...
              {assetChoice.asset.map((choice,i)=>(
                   <div onClick={()=>handleSetImage(choice)} className='p-2' key={i}>
                     <Image   src={choice.img} width={"300px"} height={"300px"} className=""/>
                   </div>
                ))}
